...
class boltz:
    ...
    def download(self,url:str,master):
        ...
        if self.is_valid_url(url) == True:
            ...
            widget = tkinter.Text(newWindow)
            newWindow.title(plname)
    
            newWindow.geometry("840x470")
    
            tkinter.Label(newWindow,text = plname, font=('Ariel',30)).pack()
            widget.pack()
            ...
            self.find_download(item,widget)

    def set_tags(self,spItem, filename, index, widget):
        widget.insert(tkinter.END,f"\nconverting ...")
        try:
            song_file = MP3(filename, ID3=EasyID3)
        except mutagen.MutagenError as e:
            widget.insert(tkinter.END,"\nFailed to download: {filename}, please ensure YouTubeDL is up-to-date. ")
            return
    ...
    def find_download(self, spItem, widget):
        for song in spItem.itemSongs:
            ...
            with yt_dlp.YoutubeDL(ydlOpts) as ydl:
                try:
                    ...
                    widget.insert(tkinter.END,"\n" + str(e))
                    messagebox.showerror("showerror", f"failed to download {name} update yt_dlp")
            self.set_tags(spItem, mp3FileName, index, widget)
            index += 1
    ...
    def get_item_name(self,spClient, itemType, itemId):
        if itemType == "playlist":
            ...
            name = spClient.album(album_id=itemId).get("name")
        elif itemType == "track":
            name = spClient.track(track_id=itemId).get("name")
    ...
    def is_valid_url(self,url:str) -> bool:
        ...
        if itemType not in ["album", "track", "playlist"]:
            ...
            messagebox.showerror("showerror", f"only tracks are supported not {itemType}")
            return False
    ...
    def parse_url(self,url:str) -> str:
        ...
        if url.startswith("spotify:"):
            ...
            messagebox.showerror("showerror", "Invalid url")
            return "ERROR", "ERROR"
    ...
    def create_sp_client(self,clientId:str, clientSecret:str):
        ...
        try:
            ...
            messagebox.showerror("showerror", "Unable to create client pls contact robesckey")
            logError(e)
            sys.exit(-1)
